...
def registro():
    ...
    try:
        ...
        print(f"Error en BD: {str(e)}", flush=True)
...
def login():
    ...
    try:
        ...
        if len(res.data) == 0:
            return jsonify({"status": "error", "message": "No encontramos este correo en el Refugio."}), 404
        
        usuario = res.data[0]
        hash_guardado = usuario.get('contrasena')

        if not hash_guardado:
            return jsonify({"status": "error", "message": "Error de base de datos: Contraseña nula."}), 500
        ...
        print(f"Error Login: {str(e)}", flush=True)
...
def obtener_datos():
    ...
    try:
        ...
        print(f"Error Datos: {str(e)}", flush=True)
        return jsonify({"status": "error", "message": "Error al buscar datos."}), 500
